...
const ReportsAnalytics = () => {
  ...
  const [activeSection, setActiveSection] = useState('reports'); // 'reports' or 'commission'
  ...
  const monthlyBookingData = {
    ...
    datasets: [
      {
        label: 'Hotels',
        data: MONTHS.map(m => (monthlyByType[m] || {}).hotel || 0),
        backgroundColor: '#093C5D',
        borderRadius: 4
      },
      {
        label: 'Vehicles',
        data: MONTHS.map(m => (monthlyByType[m] || {}).vehicle || 0),
        backgroundColor: '#3B7597',
        borderRadius: 4
      },
      {
        label: 'Guides',
        data: MONTHS.map(m => (monthlyByType[m] || {}).guide || 0),
        backgroundColor: '#5DF8D8',
        borderRadius: 4
      },
      {
        label: 'Tours',
        data: MONTHS.map(m => (monthlyByType[m] || {}).tour || 0),
  ...
  const commissionLineData = {
    ...
    datasets: [
      {
        label: 'Hotel (25%)',
        data: MONTHS.map(m => (monthlyBreakdown[m] || {}).hotel || 0),
        borderColor: '#093C5D',
        backgroundColor: 'rgba(9,60,93,0.1)',
        fill: true,
        tension: 0.4,
        pointRadius: 4
      },
      {
        label: 'Guide (25%)',
        data: MONTHS.map(m => (monthlyBreakdown[m] || {}).guide || 0),
        borderColor: '#5DF8D8',
        backgroundColor: 'rgba(93,248,216,0.1)',
        fill: true,
        tension: 0.4,
        pointRadius: 4
      },
      {
        label: 'Vehicle (15%)',
        data: MONTHS.map(m => (monthlyBreakdown[m] || {}).vehicle || 0),
        borderColor: '#F59E0B',
  ...
  // Stat cards for Reports section
  const statCards = [
    ...
    <div id="report-content">
      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition ${
                activeSection === 'reports' ? 'bg-white shadow text-primary' : 'text-gray-500 hover:text-primary'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-medium transition ${
                activeSection === 'commission' ? 'bg-white shadow text-primary' : 'text-gray-500 hover:text-primary'
              }`}
            >
          ...
          <button
            onClick={refreshData}
            className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-secondary transition shadow-sm"
          >
