...
import { 
  ...
  Menu, X, User, BarChart3, TrendingUp
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <main className={`${isSidebarOpen ? 'ml-64' : 'ml-20'} flex-1 transition-all duration-300`}>
        ...
        <div className="p-6">
          {/* If the route is exactly /admin, show dashboard stats (no bookings table) */}
          {location.pathname === '/admin' && (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Link to="/admin/hotels" className="bg-primary text-white text-center py-3 rounded-xl">Manage Hotels</Link>
                <Link to="/admin/vehicles" className="bg-secondary text-white text-center py-3 rounded-xl">Manage Vehicles</Link>
                <Link to="/admin/guides" className="bg-dark text-white text-center py-3 rounded-xl">Manage Guides</Link>
              </div>
            </>
          )}
          {/* For all child routes (including /admin/bookings), render the Outlet */}
          <Outlet />
        </div>
      </main>
    </div>
