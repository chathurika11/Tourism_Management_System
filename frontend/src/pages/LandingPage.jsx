...
const LandingPage = () => {
  ...
  // Get all items sorted by highest rating
  ...
  return (
    <div>
      ...
      <section className="relative h-[600px] flex items-center justify-center bg-cover bg-center" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1570197579656-c8c8e6da1a2b?w=1600)' }}>
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div>
              ...
              <p className="text-gray-600 mt-2">Discover Sri Lanka's most loved destinations (Highest Rated First)</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {popularTours.map(tour => (
              <div key={tour.id} className="card cursor-pointer transform hover:scale-105 transition-all duration-300 h-full flex flex-col" onClick={() => handleTourClick(tour.id)}>
                ...
                <div className="p-4 flex-grow flex flex-col">
                  ...
                  <p className="text-gray-600 text-xs mb-2 line-clamp-2">{tour.description}</p>
                  <div className="flex justify-between items-center mt-auto pt-2">
                    <span className="font-bold text-secondary">Rs: {tour.price.toLocaleString()}</span>
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div>
              ...
              <p className="text-gray-600 mt-2">Stay at Sri Lanka's finest accommodations (Highest Rated First)</p>
            </div>
            <Link to="/hotels" className="text-secondary hover:text-primary flex items-center gap-1">View All <ArrowRight size={16} /></Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {popularHotels.map(hotel => (
              <div key={hotel.id} className="card cursor-pointer transform hover:scale-105 transition-all duration-300 h-full flex flex-col" onClick={() => handleHotelClick(hotel.id)}>
                ...
                <div className="p-4 flex-grow flex flex-col">
                  ...
                  <div className="flex justify-between items-center mt-auto pt-2">
                    <span className="font-bold text-secondary">Rs {hotel.pricePerNight.toLocaleString()} <span className="text-xs font-normal">/ night</span></span>
                    <button className="text-accent hover:text-primary text-sm">View Details →</button>
                  </div>
                </div>
              </div>
      ...
      {/* Popular Vehicles for Rent */}
      <section className="py-16 bg-cream">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div>
              <h2 className="text-3xl font-bold text-primary">Vehicles for Rent</h2>
              <p className="text-gray-600 mt-2">Choose from our most popular vehicles (Highest Rated First)</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {popularVehicles.map(vehicle => (
              <div key={vehicle.id} className="card cursor-pointer transform hover:scale-105 transition-all duration-300 h-full flex flex-col" onClick={() => handleVehicleClick(vehicle.id)}>
                ...
                <div className="p-4 flex-grow flex flex-col">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-1 bg-yellow-50 px-2 py-0.5 rounded-full">
                      <Star size={14} className="text-cta fill-current" />
                      <span className="text-sm font-semibold">{vehicle.rating || '4.5'}</span>
                    </div>
                  </div>
                  <p className="text-gray-600 my-1 text-sm">{vehicle.type} • {vehicle.location}</p>
                  <div className="flex justify-between items-center mt-auto pt-2">
                    <span className="font-bold text-secondary">Rs: {vehicle.pricePerDay.toLocaleString()} <span className="text-xs font-normal">/ day</span></span>
                    <button className="text-accent hover:text-primary text-sm">View Details →</button>
                  </div>
                </div>
              </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center mb-12">
            <div>
              ...
              <p className="text-gray-600 mt-2">Meet our most requested local guides (Highest Rated First)</p>
            </div>
            <Link to="/guides" className="text-secondary hover:text-primary flex items-center gap-1">View All <ArrowRight size={16} /></Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {popularGuides.map(guide => (
              <div key={guide.id} className="card cursor-pointer text-center p-4 transform hover:scale-105 transition-all duration-300 h-full flex flex-col" onClick={() => handleGuideClick(guide.id)}>
                ...
                <div className="flex items-center justify-center gap-1 mt-2">
                  ...
                  <span className="text-xs text-gray-400 ml-1">({guide.reviews} reviews)</span>
                </div>
      ...
      <section className="py-16 bg-secondary text-white text-center">
        <div className="container mx-auto px-4">
          ...
          <Link to="/register" className="bg-cta text-primary px-8 py-3 rounded-lg font-bold hover:bg-opacity-90 transition inline-block">
            Create Free Account
          </Link>
        </div>
      </section>
