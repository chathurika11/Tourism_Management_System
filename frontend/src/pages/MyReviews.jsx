...
const MyReviews = () => {
  ...
  const allFeedbacks = [
    ...
    ...(data?.tourFeedbacks || []).map(fb => ({ ...fb, type: 'Tour', itemName: fb.tour?.name, image: fb.tour?.image })),
        ...
        <div className="space-y-6">
          {allFeedbacks.map(fb => (
            <div key={fb.id} className="bg-white rounded-xl shadow-md p-6">
              <div className="flex items-center gap-4"><img src={getImageUrl(fb.image)} className="w-16 h-16 object-cover rounded-lg" /><div><p className="text-sm text-gray-500">{fb.type}</p><h3 className="font-bold text-lg text-primary">{fb.itemName}</h3><div className="flex gap-1 mt-1">{[...Array(5)].map((_,i) => (<Star key={i} size={16} className={i < fb.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'} />))}</div></div></div>
